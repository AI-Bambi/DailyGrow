...
## プロジェクト概要
毎日の小さな積み上げを可視化し、継続をプロセス重視でサポートする習慣形成アプリケーション。結果よりも「続けること」を大切にし、ポジティブなフィードバックでモチベーションを維持する。
...
### 2. 行動の褒賞システム
- **結果より継続のプロセスを褒める**（プロセス重視）
- 毎日の行動を認識し、ポジティブなメッセージとコンフェッティで祝う
- チェックイン後にメモ入力シート（「どうだった？」感想・気づきを記録）
...
### 6. 期限管理
- 目標ごとに期限日を任意設定
- ホーム画面に残日数をカウントダウン表示（近づくほど暖色で強調）
- 期限内達成時は特別セレブレーション（confetti 200個、🎯 アイコン）
